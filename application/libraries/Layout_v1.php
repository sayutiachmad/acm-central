...
class Layout
{
	...
	private $login_view = "login/login";
    ...
    public function get_page_permission(){
        ...
        $base_nav = $this->get_base_navigation();


        $page = $base_nav;
        if($base_nav == ''){
            $cls = $this->CI->uri->segment(1);
            $met = $this->CI->uri->slash_segment(2,'leading');
            // $page = str_replace('/', '', $cls).($met==='/' ? '' : $met);    
            $page = str_replace('/', '', $cls);    
        }
        ...
        $result = $this->CI->gm_->check_page_permission_by_navigation($page,$this->sess[SESSION_USER_ROLE]);
        return $result;
    }
